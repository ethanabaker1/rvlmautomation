...
if exist('R1_Ch3')==1
    ...
    light_time_start = R1_Ch3.start; 
    light_time_interval = R1_Ch3.interval;
    light_time_length = R1_Ch3.length;
    light_values = R1_Ch3.values;
    ...
    light_time_data = horzcat(light_timematrix,R1_Ch3.values); %Constructs 2 column array (Time|Value)

    %Generate Table-Time matrix
    table_time_start = R1_Ch2.start;
    table_time_interval = R1_Ch2.interval;
    table_time_length = R1_Ch2.length;
    table_values = R1_Ch2.values;
    ...
    table_time_data = horzcat(table_timematrix,R1_Ch2.values);

    %Generate Carotid-Time Matricies
    carotid_time_start = R1_Ch1.start;
    carotid_time_interval = R1_Ch1.interval;
    carotid_time_length = R1_Ch1.length;
    ...
    carotid_time_data = horzcat(carotid_timematrix,R1_Ch1.values);
...
if exist('R2_Ch3')==1
    %Generate light-time matrix
    light_time_start = R2_Ch3.start; 
    light_time_interval = R2_Ch3.interval;
    light_time_length = R2_Ch3.length;
    light_values = R2_Ch3.values;
    light_end_time = light_time_length/(1/light_time_interval);
    light_timematrix = (light_time_start:light_time_interval:light_end_time); %Generates an array with time based on start time, end time, recording interval.
    light_timematrix = light_timematrix' ; %Transposes light array.
    light_time_data = horzcat(light_timematrix,R2_Ch3.values); %Constructs 2 column array (Time|Value)

    %Generate Table-Time matrix
    table_time_start = R2_Ch2.start;
    table_time_interval = R2_Ch2.interval;
    table_time_length = R2_Ch2.length;
    table_values = R2_Ch2.values;
    table_end_time = table_time_length/(1/table_time_interval);
    table_timematrix = (table_time_start:table_time_interval:table_end_time);
    table_timematrix = table_timematrix' ;
    table_time_data = horzcat(table_timematrix,R2_Ch2.values);

    %Generate Carotid-Time Matricies
    carotid_time_start = R2_Ch2.start;
    carotid_time_interval = R2_Ch2.interval;
    carotid_time_length = R2_Ch2.length;
    carotid_end_time = carotid_time_length/(1/carotid_time_interval);
    carotid_timematrix = (carotid_time_start:carotid_time_interval:carotid_end_time);
    carotid_timematrix = carotid_timematrix' ;
    carotid_time_data = horzcat(carotid_timematrix,R2_Ch1.values);
...
if exist('R3_Ch3')==1
    %Generate light-time matrix
    light_time_start = R3_Ch3.start; 
    light_time_interval = R3_Ch3.interval;
    light_time_length = R3_Ch3.length;
    light_values = R3_Ch3.values;
    light_end_time = light_time_length/(1/light_time_interval);
    light_timematrix = (light_time_start:light_time_interval:light_end_time); %Generates an array with time based on start time, end time, recording interval.
    light_timematrix = light_timematrix' ; %Transposes light array.
    light_time_data = horzcat(light_timematrix,R3_Ch3.values); %Constructs 2 column array (Time|Value)

    %Generate Table-Time matrix
    table_time_start = R3_Ch2.start;
    table_time_interval = R3_Ch2.interval;
    table_time_length = R3_Ch2.length;
    table_values = R3_Ch2.values;
    table_end_time = table_time_length/(1/table_time_interval);
    table_timematrix = (table_time_start:table_time_interval:table_end_time);
    table_timematrix = table_timematrix' ;
    table_time_data = horzcat(table_timematrix,R3_Ch2.values);

    %Generate Carotid-Time Matricies
    carotid_time_start = R3_Ch1.start;
    carotid_time_interval = R3_Ch1.interval;
    carotid_time_length = R3_Ch1.length;
    carotid_end_time = carotid_time_length/(1/carotid_time_interval);
    carotid_timematrix = (carotid_time_start:carotid_time_interval:carotid_end_time);
    carotid_timematrix = carotid_timematrix' ;
    carotid_time_data = horzcat(carotid_timematrix,R3_Ch1.values);
...
if exist('R4_Ch3') ==1
    %Generate light-time matrix
    light_time_start = R4_Ch3.start; 
    light_time_interval = R4_Ch3.interval;
    light_time_length = R4_Ch3.length;
    light_values = R4_Ch3.values;
    light_end_time = light_time_length/(1/light_time_interval);
    light_timematrix = (light_time_start:light_time_interval:light_end_time); %Generates an array with time based on start time, end time, recording interval.
    light_timematrix = light_timematrix' ; %Transposes light array.
    light_time_data = horzcat(light_timematrix,R4_Ch3.values); %Constructs 2 column array (Time|Value)

    %Generate Table-Time matrix
    table_time_start = R4_Ch2.start;
    table_time_interval = R4_Ch2.interval;
    table_time_length = R4_Ch2.length;
    table_values = R4_Ch2.values;
    table_end_time = table_time_length/(1/table_time_interval);
    table_timematrix = (table_time_start:table_time_interval:table_end_time);
    table_timematrix = table_timematrix' ;
    table_time_data = horzcat(table_timematrix,R4_Ch2.values);

    %Generate Carotid-Time Matricies
    carotid_time_start = R4_Ch1.start;
    carotid_time_interval = R4_Ch1.interval;
    carotid_time_length = R4_Ch1.length;
    carotid_end_time = carotid_time_length/(1/carotid_time_interval);
    carotid_timematrix = (carotid_time_start:carotid_time_interval:carotid_end_time);
    carotid_timematrix = carotid_timematrix' ;
    carotid_time_data = horzcat(carotid_timematrix,R4_Ch1.values);
...
if exist('R5_Ch3')==1
    %Generate light-time matrix
    light_time_start = R5_Ch3.start; 
    light_time_interval = R5_Ch3.interval;
    light_time_length = R5_Ch3.length;
    light_values = R5_Ch3.values;
    light_end_time = light_time_length/(1/light_time_interval);
    light_timematrix = (light_time_start:light_time_interval:light_end_time); %Generates an array with time based on start time, end time, recording interval.
    light_timematrix = light_timematrix' ; %Transposes light array.
    light_time_data = horzcat(light_timematrix,R5_Ch3.values); %Constructs 2 column array (Time|Value)

    %Generate Table-Time matrix
    table_time_start = R5_Ch2.start;
    table_time_interval = R5_Ch2.interval;
    table_time_length = R5_Ch2.length;
    table_values = R5_Ch2.values;
    table_end_time = table_time_length/(1/table_time_interval);
    table_timematrix = (table_time_start:table_time_interval:table_end_time);
    table_timematrix = table_timematrix' ;
    table_time_data = horzcat(table_timematrix,R5_Ch2.values);

    %Generate Carotid-Time Matricies
    carotid_time_start = R5_Ch1.start;
    carotid_time_interval = R5_Ch1.interval;
    carotid_time_length = R5_Ch1.length;
    carotid_end_time = carotid_time_length/(1/carotid_time_interval);
    carotid_timematrix = (carotid_time_start:carotid_time_interval:carotid_end_time);
    carotid_timematrix = carotid_timematrix' ;
    carotid_time_data = horzcat(carotid_timematrix,R5_Ch1.values);
...
if exist('R6_Ch3')==1
    %Generate light-time matrix
    light_time_start = R6_Ch3.start; 
    light_time_interval = R6_Ch3.interval;
    light_time_length = R6_Ch3.length;
    light_values = R6_Ch3.values;
    light_end_time = light_time_length/(1/light_time_interval);
    light_timematrix = (light_time_start:light_time_interval:light_end_time); %Generates an array with time based on start time, end time, recording interval.
    light_timematrix = light_timematrix' ; %Transposes light array.
    light_time_data = horzcat(light_timematrix,R6_Ch3.values); %Constructs 2 column array (Time|Value)

    %Generate Table-Time matrix
    table_time_start = R6_Ch2.start;
    table_time_interval = R6_Ch2.interval;
    table_time_length = R6_Ch2.length;
    table_values = R6_Ch2.values;
    table_end_time = table_time_length/(1/table_time_interval);
    table_timematrix = (table_time_start:table_time_interval:table_end_time);
    table_timematrix = table_timematrix' ;
    table_time_data = horzcat(table_timematrix,R6_Ch2.values);

    %Generate Carotid-Time Matricies
    carotid_time_start = R6_Ch1.start;
    carotid_time_interval = R6_Ch1.interval;
    carotid_time_length = R6_Ch1.length;
    carotid_end_time = carotid_time_length/(1/carotid_time_interval);
    carotid_timematrix = (carotid_time_start:carotid_time_interval:carotid_end_time);
    carotid_timematrix = carotid_timematrix' ;
    carotid_time_data = horzcat(carotid_timematrix,R6_Ch1.values);
